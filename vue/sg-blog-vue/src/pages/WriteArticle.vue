<template>
    <div>
        <sg-navbar></sg-navbar>
        <div class="app-container" style="background-color: #fff; margin-top: 60px; max-width: 1000px; margin-left: auto; margin-right: auto;">
  ...
  <script>
  import { listAllCategory, listAllTag  } from '@/api/category'
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit() {
        if (!this.aId) {
          this.form.status = '0'
          console.log("post请求的博客内容", this.form);
          addArticle(this.form).then(response => {
